...
@test "encrypts file to same path when no output_path (second arg) is given" {
  local input_path="$REPO_ROOT/test.txt"
  touch "$input_path"
...
@test "encrypts file to given output_path" {
  ...
  local output_path="$REPO_ROOT/test.txt"
...
EOF
  ...
  local input_path="$REPO_ROOT/test_multi.txt"
  touch "$input_path"
...
@test "does not leave unencrypted file when encryption fails" {
  ...
  local input_path="$REPO_ROOT/test_secure.md"
  touch "$input_path"
